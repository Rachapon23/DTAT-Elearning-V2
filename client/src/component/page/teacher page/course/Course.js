...
const Course = () => {
	...
	const handdleSubmit = async (e) => {
		...
		if (nameCourse.statuscourse && !!!nameCourse.room) {
			valid = false;
			document.getElementById("room").focus({ focusVisible: true });
		}
		if (nameCourse.calendar.length === 0 && nameCourse.statuscourse) {
			// setError({room: "Please select room"})
			valid = false;
			Swal.fire({
				icon: "error",
		...
		if (nameCourse.statuscourse) {
            for (let i = 0; i < nameCourse?.member.length; i++) {
                if (nameCourse?.member[i]?.plant === "") {
                    valid = false;
                    Toast.fire({
                        icon: 'error',
